angular.module('app').factory('evaFormReplyService', ['httpService', '$q',function(httpService,$q){
    var service = {};
    service.getFormConfig = function(){
         return httpService.apiRequest("GET", "/api/formConfig/1/1"); 
    ...
    return service; 
    function mergeData(formConfig,replyData){
        formConfig,replyData;
        if( replyData.detail!==null ){
            for( var i=0;i<formConfig.item_group.length;i++ ){
                for( var j=0;j<formConfig.item_group[i].item.length;j++ ){
                    for( var z=0;z<replyData.detail.length;z++ ) {
                        var item=formConfig.item_group[i].item[j]; 
                        var draft=replyData.detail[z];
                        if( formConfig.item_group[i].id===draft.review_item_group_id&&
                              ...
                              //console.log(replyData.detail[z].review_value);     
                        }
                    }         
                }
           }
    ...
    function submit(result,isStatus,query,isUpdate){
        var apiUrl = "";
        if( isUpdate ){
            apiUrl = "/api/updateReviewReply";
        }else{
            apiUrl = "/api/createReviewReply";
        }
        ...
        myForm.append('isStatus',isStatus);
